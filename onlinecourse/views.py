...
def show_exam_result(request, course_id, submission_id):
    ...
    choices = Submission.objects.get(id=submission_id).choices.all()
    questions = Question.objects.filter(course=course_id)
    total_score = 0
    max_score = 0

    for choice in choices:
        max_score += choice.question.question_grade
        if choice.is_correct:
            total_score+= choice.question.question_grade

    total_score_percent = round(total_score / max_score * 100)
    context = {
        "course": course,
        "choices": choices,
        "grade": total_score_percent,
        "questions": questions,
    }
